# C�digo de ejemplo para crear un validador
...
def mostrar_ventana_progreso(titulo, max_val):
    ...
    # Crear y colocar una etiqueta para mostrar el texto de la funci�n en ejecuci�n
...
def loadExcel():
    ...
    # Retornar los encabezados y la hoja de c�lculo
    return headers, sheet
    
def chooseBase(base):
    switch = {
        "nna": nna,  #ejemplo de insercion de cada base para validar 
    }
    execute_validator = switch.get(base)
    execute_validator()
    
def nna(): # funcion para determinar la cantdad de paginas a validar 
    # P�ginas del archivo Excel cargado
    num_paginas = len(workbook.sheetnames)
    print(f"El archivo Excel tiene {num_paginas} p�ginas.")

    # Primero, validar la p�gina 1
    if num_paginas >= 1 and workbook.sheetnames[0] in workbook.sheetnames:
        sheet = workbook[workbook.sheetnames[0]]  # Acceder a la p�gina 1
        print("Validando la p�gina 1...")
        csapag1(sheet)
    ...
    try:
        ...
        for i in range(2, ultima_fila + 1):
            ...
            if len(sheet.cell(i, 8).value.strip()) > 0 and len(sheet.cell(i, 9).value.strip()) > 0: # condicion para validar las celdas 
                ...
                colum["column"] = {8, 9, 2}
                colum["row"] = i
                pintar(colum, sheet)# funci�n para pintar las celdas establecidas 
                

            # Codigo generado automaticamente
            # valor para validar vacias
            CeldasVacias['vacias'] = {5,8}
            ...
            if progress['value'] == 100:
                ventana.after(100, lambda: ventana.destroy())  # Cerrar la ventana despu�s de 100 ms
                break
            #/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
def saveFile():
    try:
        ...
        # Guardar el archivo modificado con el nombre espec�fico para errores
...
def preguntaDescarga():
    try:
        ...
        if respuesta == "yes":
            ...
            print("Tu archivo no ser� descargado")
    except Exception as e:
